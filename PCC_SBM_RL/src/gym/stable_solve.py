...
print("gamma = %f" % gamma)
model = PPO1(MyMlpPolicy, env, verbose=1, schedule='constant', timesteps_per_actorbatch=8192, 
                                                            optim_batchsize=2048, gamma=gamma)
"""
# Dry run execution of the trained model
for i in range(0, 1):
    with model.graph.as_default():                                                                   
        saver = tf.train.Saver()                                                                     
        saver.save(training_sess, "./pcc_model_%d.ckpt" % i)
    model.learn(total_timesteps=410)

"""

### Original training loop from github
for i in range(0, 6):
    with model.graph.as_default():                                                                   
        saver = tf.train.Saver()                                                                     
        saver.save(training_sess, "./pcc_model_%d.ckpt" % i)
